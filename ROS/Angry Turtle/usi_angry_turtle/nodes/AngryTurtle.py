...
def proximity(listener, turtle_vel):
    k2 = 1
    (trans,rot) = listener.lookupTransform('/turtle2','/turtle1', rospy.Time(0))
    if(math.sqrt(trans[0] ** 2 + trans[1] ** 2)<k2):
	pursue(listener, turtle_vel)

def pursue(listener, turtle_vel):
    while not rospy.is_shutdown():
	(trans,rot) = listener.lookupTransform('/turtle2','/turtle1', rospy.Time(0))
...
if __name__ == '__main__':
    ...
    turtle_set_pen = rospy.ServiceProxy('turtle2/set_pen', SetPen)
    x = TurtleBot()
    while not rospy.is_shutdown():
	try:	
	    turtle_set_pen(0,0,0,0,1)
	    proximity(listener, turtle_vel)
	    x.move2goal(1,9)
	    proximity(listener, turtle_vel)
